...
```
ingest:  provider Fetcher ──► []model.Record ──► summary.NewPlan ──► Plan.Conversations ──► ir.Exporter
           (per-provider)      (normalized;        (metadata-only      (materialize:         (streamed to disk,
                                bodies may be       grouping, zero      reconstruct, yield)   one conv at a time)
                                lazy)               body loads)

view:    ir.OpenDir ──► serve.Server ──► localhost web UI (manifest for the index; one conv file per request)
```
...
The IR directory is the single ingestion output and the durable seam between
ingest and view. `summary.Summarize` survives as the retain-everything wrapper
the IR golden tests use.

## Project structure

```
cmd/dressage/main.go      - cobra CLI: provider subcommands (ingest → IR via --out), serve subcommand, shared runReport
internal/
  model/                  - provider-neutral data types (only logic: Body.Load/Present lazy indirection)
    ...
    summary.go            - Report, DaySummary, ConversationSummary (+Sidechains), Invocation, Stats (export model)
  ...
  ir/                     - IR exporter+reader: ir.go (schema types), map.go (pure model→ir mappers),
                            facts.go/turns.go (Parquet row mappers), export.go (streaming Exporter +
                            manifest, the write IO), read.go (Reader: OpenDir/LoadConversation, the read IO)
  serve/                  - dressage serve: net/http + html/template web UI over an ir.Reader
                            (server.go, handlers.go, viewmodel.go, format.go, templates/*.tmpl via embed.FS)
docs/
...
- **One output: the IR.** The streaming `ir.Exporter` is the sole ingestion
  sink, and the durable external contract: versioned (`dressage.ir/1.2`),
  snake_case JSON per conversation (reconstructed view + sidechains +
  invocations) + `manifest.json` + two Parquet tables (`facts.parquet` per
  invocation, `turns.parquet` per deduplicated turn incl. sidechains). Raw wire
  bodies are opt-in (`--raw-bodies embed`); the manifest records the mode.
- **Viewing is separate from ingest.** `dressage serve` (`internal/serve`) reads
  an IR directory back through `ir.Reader` and renders a localhost web UI — the
  replacement for the retired HTML report. It never re-ingests; the IR seam is
  the only thing between the two halves.
...
- **Tests** are table-driven with **golden files**:
  ...
  `internal/serve` tests drive the handler with `httptest` against a real IR
  export of a fixture (substring assertions, not an HTML byte-golden). Fetchers
  use small fake API interfaces (e.g. `s3API`) for testability — follow that
  pattern.
- **`internal/ir`** keeps `model→ir` translation in pure mappers (`map.go`,
  ...
  `model.Body.Load`); all IO is isolated to `export.go` (`Exporter`, writes)
  and `read.go` (`Reader`, reads). On-disk filenames are a sanitized transform
  of the conversation id; consumers (including `internal/serve`) must resolve
  files via the manifest `file`/`files` fields, never by rebuilding from id.
...
- **Serve from the IR, don't render a flat file** — the static self-contained
  HTML report was retired because it did not scale: a resend-style capture
  produced a 2.4 GB single file (every turn re-embeds the growing transcript).
  `dressage serve` renders the same content from the IR, reading the manifest
  for the index and one conversation file per request, so a viewer's memory is
  bounded to a single conversation regardless of capture size. Pages are
  server-rendered `html/template` (auto-escaped), zero JS, `<details>` drill-down.
...
- **`json.RawMessage` for bodies, behind a lazy handle** — request/response
  bodies vary by model and operation, so they are carried raw and embedded
  inline in the IR when requested. `Body.Source` (`BodySource.Load()`) lets a
  fetcher defer the payload to disk; all readers go through
  `Body.Load()`/`Present()`.
- **Bounded memory, not streaming fetch** — `Fetch` still returns the whole
  ...
  materialize one at a time into the streaming IR exporter, retaining nothing.
  Peak memory ≈ the largest single transcript (measured: the whole 25 GB claude
  capture exports with ~700 MB peak).
- **IR is versioned and provider-neutral** — both `dressage serve` and any
  downstream analysis program consume conversations without re-parsing
  provider-native logs.
- **Body preview cap in serve** — `serve.prettyBody` truncates each raw
  invocation body shown inline to 32 KiB with a marker (the full body is one
  click away at the `…/raw/{idx}/{dir}` endpoint). This keeps a page fast for
  providers (like `claude`) that resend the full running transcript every turn;
  reconstruction and the IR are unaffected since they read the raw JSON.
